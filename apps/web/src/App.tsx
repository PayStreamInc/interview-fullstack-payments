...
export function App() {
  ...
  async function claimRefund(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload: ClaimRefundRequest = {
      ...
      accountType: String(formData.get("accountType") ?? "checking") as ClaimRefundRequest["accountType"],
    };
    ...
    <main className="mx-auto flex min-h-screen max-w-2xl items-center px-4 py-10">
      <Card className="w-full">
        <CardHeader>
          ...
          <p className="text-sm text-slate-600">Select an unclaimed refund, then enter ACH details.</p>
        </CardHeader>
        <CardContent className="space-y-6">
          <section className="space-y-3">
            {isLoading ? <p className="text-sm text-slate-600">Loading refunds...</p> : null}
            {!isLoading && refunds.length === 0 ? <Alert>No unclaimed refunds available.</Alert> : null}
            <div className="space-y-2">
              {refunds.map((refund) => (
                <button
                  className={`w-full rounded-lg border p-3 text-left text-sm transition-colors ${
                    selectedRefund?.id === refund.id ? "border-primary bg-muted" : "border-border bg-white hover:bg-muted"
                  }`}
                  key={refund.id}
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <span className="font-medium">${(refund.amountCents / 100).toFixed(2)} {refund.currency}</span>
          ...
          <section className="space-y-3 border-t border-border pt-6">
            <div>
              ...
              <p className="text-sm text-slate-600">
                Selected refund: {selectedRefund ? `$${(selectedRefund.amountCents / 100).toFixed(2)} ${selectedRefund.currency}` : "none"}
              </p>
            </div>
          ...
          <form className="space-y-4" onSubmit={claimRefund}>
            ...
            <Button className="w-full" disabled={isSubmitting || selectedRefund?.status !== "unclaimed"}>
